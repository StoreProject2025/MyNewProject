...
<<<<<<< HEAD
        ...
        $('.add-to-cart-btn').on('click', function(e) {
            ...
            console.log('Add to Cart button clicked for product:', productSlug);
            
            // Disable the button while processing
            button.prop('disabled', true);
            
            $.ajax({
                url: `/cart/add/${productSlug}`,
                type: 'POST',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                success: function(response) {
                    ...
                    if (response.success) {
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'خطأ!',
                            text: response.message || 'حدث خطأ أثناء إضافة المنتج إلى السلة',
                            showConfirmButton: false,
                            timer: 1500
                        });
                    }
        ...
        function updateCartCount(count) {
            ...
            let cartCountBadge = cartIcon.find('.cart-count');
            ...
            if (count > 0) {
                if (cartCountBadge.length) {
                    ...
                    // Create new badge
                    cartIcon.append(`<span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger cart-count">${count}</span>`);
                }
            } else {
